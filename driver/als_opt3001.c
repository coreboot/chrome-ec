...
int opt3001_read_lux(const struct motion_sensor_t *s, vector_3_t v)
{
	...
	ret = opt3001_i2c_read(I2C_PORT_ALS, s->addr, OPT3001_REG_RESULT, &data);
	if (ret)
		return ret;
			     ...
			     int rnd)
{
	...
	rv = opt3001_i2c_read(I2C_PORT_ALS, s->addr, OPT3001_REG_CONFIGURE, &reg);
	if (rv)
		return rv;

	rv = opt3001_i2c_write(I2C_PORT_ALS, s->addr, OPT3001_REG_CONFIGURE,
...
static int opt3001_init(const struct motion_sensor_t *s)
{
	...
	ret = opt3001_i2c_read(I2C_PORT_ALS, s->addr, OPT3001_REG_MAN_ID, &data);
	if (ret)
		return ret;
	if (data != OPT3001_MANUFACTURER_ID)
		return EC_ERROR_ACCESS_DENIED;

	ret = opt3001_i2c_read(I2C_PORT_ALS, s->addr, OPT3001_REG_DEV_ID, &data);
	if (ret)
		return ret;
	if (data != OPT3001_DEVICE_ID)
		return EC_ERROR_ACCESS_DENIED;
	 ...
	 * [4]    : 1b    Latched window-style comparison operation
	 */
	 opt3001_i2c_write(I2C_PORT_ALS, s->addr, OPT3001_REG_CONFIGURE, 0x810);
