...
static int set_range(const struct motion_sensor_t *s, int range, int rnd)
{
	...
	ret = raw_read8(I2C_PORT_ACCEL, s->addr, reg, &range_reg_val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	reg_val = (range_reg_val & ~BMA2x2_RANGE_SELECT_MSK) | range_val;
	ret = raw_write8(I2C_PORT_ACCEL, s->addr, reg, reg_val);
...
static int set_data_rate(const struct motion_sensor_t *s, int rate, int rnd)
{
	...
	/* Determine the new value of control reg and attempt to write it. */
	ret = raw_read8(I2C_PORT_ACCEL, s->addr, reg, &odr_reg_val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
...
static int read(const struct motion_sensor_t *s, vector_3_t v)
{
	...
	mutex_lock(s->mutex);
	ret = raw_read_multi(I2C_PORT_ACCEL, s->addr, BMA2x2_X_AXIS_LSB_ADDR,
			     acc, 6);
	mutex_unlock(s->mutex);
...
static int init(const struct motion_sensor_t *s)
{
	...
	ret = raw_read8(I2C_PORT_ACCEL, s->addr, BMA2x2_CHIP_ID_ADDR, &val);
	if (ret)
	...
	ret = raw_read8(I2C_PORT_ACCEL, s->addr, reg, &val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
	val |= reset_field;
	ret = raw_write8(I2C_PORT_ACCEL, s->addr, reg, val);
	if (ret != EC_SUCCESS) {
		mutex_unlock(s->mutex);
		return ret;
	}
