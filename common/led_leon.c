...
static int leon_led_set_bat_gpio(enum led_color color,
...
static int leon_led_set_kb_gpio(enum led_color color,
			      enum gpio_signal gpio_led_blue_l)
{
	switch (color) {
	case LED_OFF:
...
{
	return leon_led_set_bat_gpio(color, GPIO_BAT_LED0_L, GPIO_BAT_LED1_L);
}

static int leon_led_set_color_power(enum led_color color)
{
	return leon_led_set_kb_gpio(color, GPIO_KB_LED_L);
}
...
static void leon_led_set_power(int ticks)
{
...
static void leon_led_set_battery(int ticks)
{
...
static void led_tick(void)
{
	...
	if (led_auto_control_is_enabled(EC_LED_ID_POWER_LED))
		leon_led_set_power(ticks);

	if (led_auto_control_is_enabled(EC_LED_ID_BATTERY_LED))
		leon_led_set_battery(ticks);
}
