...
static int chip_i2c_xfer_with_notify(int port, int periph_addr,
				     ...
				     uint8_t *in, int in_size, int flags)
{
	...
	i2c_start_xfer_notify(port, periph_addr);
#endif

	ret = chip_i2c_xfer(port, periph_addr, out, out_size, in, in_size,
			    flags);

#ifdef CONFIG_I2C_XFER_BOARD_CALLBACK
	i2c_end_xfer_notify(port, periph_addr);
#endif
	return ret;
}
 ...
 */
static int i2c_xfer_no_retry(int port, int periph_addr, const uint8_t *out,
			     int out_size, uint8_t *in, int in_size, int flags)
{
	...
	ret = chip_i2c_xfer_with_notify(port, periph_addr, out, out_size, in,
					in_chunk_size, out_flags);
	in += in_chunk_size;
	while (in_size && ret == EC_SUCCESS) {
		in_chunk_size = MIN(in_size, CONFIG_I2C_CHIP_MAX_READ_SIZE);
		in_size -= in_chunk_size;
		ret = chip_i2c_xfer_with_notify(port, periph_addr, NULL, 0, in,
...
#ifdef CONFIG_I2C_XFER_LARGE_READ
		ret = i2c_xfer_no_retry(port, periph_addr, out, out_size, in,
			in_size, flags);
#else
		ret = chip_i2c_xfer_with_notify(port, periph_addr, out,
	     ...
	     uint8_t *in, int in_size)
{
	...
	i2c_lock(port, 1);
	rv = i2c_xfer_unlocked(port, periph_addr, out, out_size, in, in_size,
			I2C_XFER_SINGLE);
	i2c_lock(port, 0);
...
int i2c_read32(int port, int periph_addr, int offset, int *data)
{
	...
	rv = i2c_xfer(port, periph_addr, &reg, 1, buf, sizeof(uint32_t));

	if (rv)
		return rv;

	if (periph_addr & I2C_FLAG_BIG_ENDIAN)
...
int i2c_write32(int port, int periph_addr, int offset, int data)
{
	...
	return i2c_xfer(port, periph_addr, buf, sizeof(uint32_t) + 1,
			NULL, 0);
}
...
int i2c_read16(int port, int periph_addr, int offset, int *data)
{
	...
	rv = i2c_xfer(port, periph_addr, &reg, 1, buf, sizeof(uint16_t));
...
int i2c_write16(int port, int periph_addr, int offset, int data)
{
	...
	return i2c_xfer(port, periph_addr, buf, 1 + sizeof(uint16_t), NULL, 0);
}
...
int i2c_read8(int port, int periph_addr, int offset, int *data)
{
	...
	rv = i2c_xfer(port, periph_addr, &reg, 1, &buf, 1);
	if (!rv)
		*data = buf;
...
int i2c_write8(int port, int periph_addr, int offset, int data)
{
	...
	return i2c_xfer(port, periph_addr, buf, 2, 0, 0);
}
		      ...
		      int len)
{
	...
	rv = i2c_xfer(port, periph_addr, addr, 2, buf, len);
		       ...
		       int len)
{
	...
	return i2c_xfer(port, periph_addr, buf, 2 + len, NULL, 0);
}
			    ...
			    uint8_t *data, int len)
{
	...
	return i2c_xfer(port, periph_addr, addr, 2, data, len);
}
			     ...
			     const uint8_t *data, int len)
{
	 ...
	 */
	i2c_lock(port, 1);
	rv = i2c_xfer_unlocked(port, periph_addr, addr, 2, NULL, 0,
			       I2C_XFER_START);
	if (!rv)
		rv = i2c_xfer_unlocked(port, periph_addr, data, len, NULL, 0,
				       I2C_XFER_STOP);
	i2c_lock(port, 0);
		    ...
		    int len)
{
	 ...
	 */
	rv = i2c_xfer_unlocked(port, periph_addr, &reg, 1, &block_length, 1,
		      I2C_XFER_START);
	if (rv)
		goto exit;
	...
	rv = i2c_xfer_unlocked(port, periph_addr, 0, 0, data, block_length,
		      I2C_XFER_STOP);
	data[block_length] = 0;
		   ...
		   int len)
{
	...
	rv = i2c_xfer(port, periph_addr, &reg_address, 1, data, len);
	return rv;
}
		    ...
		    int len)
{
	 ...
	 */
	i2c_lock(port, 1);
	rv = i2c_xfer_unlocked(port, periph_addr, &reg_address, 1, NULL, 0,
			       I2C_XFER_START);
	if (!rv) {
		rv = i2c_xfer_unlocked(port, periph_addr, data, len, NULL, 0,
				       I2C_XFER_STOP);
...
static int command_i2cxfer(int argc, char **argv)
{
	...
	if (strcasecmp(argv[1], "r") == 0) {
		/* 8-bit read */
		if (offset_size == 2)
			rv = i2c_read_offset16(port, periph_addr, offset, &v,
					       1);
		else
			rv = i2c_read8(port, periph_addr, offset, &v);
		if (!rv)
			ccprintf("0x%02x [%d]\n", v, v);
		...
		if (offset_size == 2)
			...
			rv = i2c_read16(port, periph_addr, offset, &v);
		if (!rv)
			ccprintf("0x%04x [%d]\n", v, v);
		...
		rv = i2c_xfer(port, periph_addr, (uint8_t *)&offset, 1, data,
			      v);
		...
		if (argc < 6)
			return EC_ERROR_PARAM5;
		if (offset_size == 2)
			rv = i2c_write_offset16(port, periph_addr, offset, v,
						1);
		else
			rv = i2c_write8(port, periph_addr, offset, v);
		...
		if (offset_size == 2)
			...
			rv = i2c_write16(port, periph_addr, offset, v);
